...
module.exports = Command.extend({
    ...
    help: function () {
        ...
        r += '\n      -c,  --sanitizeRemote <boolean>\t\t' + d('true') + '\t\tTo remove redundant remote properties if not matched locally (only in YAPI mode).';
    ...
    options: {
        ...
        cwd: {//change glob to new cwd
            type: 'string',
            alias:'w'
        },
        verbose: {//console logging to debug level
            type: 'boolean',
            alias: 'v'
        },
    ...
    run: function () {
        ...
        if (!cliOpts.search || cliOpts.search.length === 0) {
            cliOpts.search = path.join(cliOpts.cwd || process.cwd(), '*.xml');
        } else {
            cliOpts.search = path.join(cliOpts.cwd || process.cwd(), cliOpts.search);
        }
        ...
        if (cliOpts.unattended === true) {
            ...
            if (prompts.length > 0) {
                inquirer.prompt(prompts,
                    function (answers) {
                        cliOpts.sanitizeRemote = answers.cleanup || cliOpts.sanitizeRemote;
