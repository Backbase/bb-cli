...
_.assign(Cmis.prototype, {
    ...
    getPath: function(pathIndex) {
        ...
        var that = this;
        this.idPromise[contentPath] = this.request('get', 'path', query)
        .then(function(jx) {
        ...
        return this.idPromise[contentPath];
    },
    ...
    create: function(pathIndex, parentObjectId) {
        ...
        var type = (pathIndex === this.path.length - 1) ? this.type : 'cmis:folder';
        // do not create for image
        if (type === 'bb:image') type = 'cmis:folder';

        var bodyArray = [
            {type: 'id', id: 'cmis:objectTypeId', value: type},
            {type: 'id', id: 'cmis:createdBy', value: this.config.username}
        ];

        var body;
        if (type === this.type) {
            // bodyArray.push({type: 'string', id: 'reference', value: this.path.join('/')});
            bodyArray.push({type: 'id', id: 'cmis:lastModifiedBy', value: this.config.username});
            body = getPostBody(name, bodyArray, this.mimeType);
        } else {
            bodyArray.push({type: 'string', id: 'cmis:name', value: name});
            body = getPostBody(name, bodyArray);
        }
...
function getPostBody(itemName, props, mimeType) {
    if (mimeType) {
        props.push({type: 'id', id: 'cmis:contentStreamMimeType', value: mimeType});
    }
    var jxProps = {};
    ...
    if (mimeType) {
        // var p = out['atom:entry']['cmisra:object']['cmis:properties'];
        // var prop = ['cmis:propertyString'];
        // var mime = {
        ...
        // };
        // if (p) {
        //     if (p[prop] instanceof Array) p.push(mime);
        //     else {
        //         p[prop] = [];
        //         p[prop].push(mime);
        //     }
        ...
        //     p[prop] = mime;
        // }
        out['atom:entry']['cmisra:content'] = {
            'cmisra:mediatype': mimeType,
            'cmisra:base64': {
                _: 'PHA+YXNkZGE8L3A+'
            }
        };
