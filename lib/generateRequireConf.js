...
function GenerateRequireConf(options, baseUrl) {
...
GenerateRequireConf.prototype.process = function(){
    ...
    bower.commands.list({}, {cwd: that.baseUrl})
        .on('end', function (dependencyGraph) {
            ...
            var libsPaths = buildConfig(dependencyGraph, that.options.bowerBuildConfig);
            ...
            bower.commands.list({paths: true}, {cwd: that.baseUrl})
                .on('end', function (dependencyPaths) {
                    ...
                    that.compareRequireConf(bowerPaths)
                        .then(function(mergedConf){
                            return that.applyWebUrl.apply(that, [mergedConf]);
                        })
...
GenerateRequireConf.prototype.applyWebUrl = function(mergedConf){
    ...
    if (__.hasPath(that.options, 'bowerOpts.custom.webUrl')) {
        ...
        _.forOwn(output.paths, function (value, key) {
            output.paths[key] = value.replace(basePath, that.options.bowerOpts.custom.webUrl);
        });
    }
