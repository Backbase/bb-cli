...
GenerateRequireConf.prototype.process = function(){
    ...
    bower.commands.list({paths: true}, {cwd: that.baseUrl})
        .on('end', function (dependencyPaths) {
            ...
            that.compareRequireConf(bowerPaths, that.options.bbRc.install.requirejsConfigs)
                .then(function(mergedConf){
                    return that.applyWebUrl.apply(that, [mergedConf, that.options.bbRc.install.requirejsBaseUrl]);
                })
 ...
 */
GenerateRequireConf.prototype.getRequireConfPaths = function(raw){
    try {
        ...
        var output = eval('var dist="";var path="";('+output+')');
...
GenerateRequireConf.prototype.readRequireConf = function(filePath){
    ...
    if (haveWar) {
        ...
        fs.readFile(path.join(this.baseUrl, pathToWar), function (err, data) {
            ...
            deferred.resolve(that.getRequireConfPaths(zip.file(pathToInWar).asText()));
        });
    } else {
        fs.readFile(filePath, 'utf-8', function (err, data) {
            if (err) throw err;

            deferred.resolve(that.getRequireConfPaths(data));
        });
    }
