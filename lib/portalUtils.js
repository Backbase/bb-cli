...
_.extend(exports, {
    ...
    getPom: function(c, p) {
        p = p || '';
        var pomPath = path.join(c.config.bbrc.path, p, '/pom.xml');
    ...
    getInfo: function(c, p) {
        ...
        .then(function(r) {
            ...
            r = r.configuration;
            var webApp = r.webApp || r.webAppConfig;
            o.jetty = {
                context: webApp.contextPath,
                port: (r.httpConnector || r.connectors.connector).port,
                defaults: webApp.defaultsDescriptor,
                resources: webApp.resourceBases.resourceBase
            };
            return o;
        });
    },
    ...
    getRequireConfs: function(c) {
        ...
        .then(function(files) {
            var a = [];
            var i;
            for (i = 0; i < files.length; i++) a.push(readFile(files[i]));
            return Q.all(a)
            .then(function(fs) {
                var o = {};
