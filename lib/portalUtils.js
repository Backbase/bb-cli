...
_.extend(exports, {
            ...
            .then(function(ra) {
                ...
                .then(function(pom) {
                    _.each(tmps, function(v, i) {
                        tmps[i] = c.config._local.path + '/target/' + pom.project.artifactId + '/WEB-INF/' +
                                  _.find(v, {$name: 'BundleName'}).value._ + '/' + _.find(v, {$name: 'Web'}).value._ +
                                  '.jsp';
                    });
                    // console.log(tmps);
                    return tmps;
    ...
    getPom: function(c, p) {
        p = p || '';
        var pomPath = c.config._local.path + p + '/pom.xml';
    ...
    getStaticFiles: function(c, fileMask) {
        return exports.getPom(c)
        .then(function(pom) {
            var staticPath = c.config._local.path + '/target/' + pom.project.artifactId + '/static/',
                dfrd = Q.defer();

            try {
                Finder.from(staticPath).findFiles(fileMask, function(files) {
                    dfrd.resolve(files);
                });
            } catch(e) {
