...
exports.exportPortal = function (xml, portalName, destination, onFileCreated) {
    ...
    var portalsData = portalsBuilder.buildObject({
        portal: parsePortal(xml.exportBundle.portals.portal)
    });
...
exports.listPortals = function (contextName, callback) {
    if (contextName && typeof contextName !== 'string') return util.err('Context name is not defined on listPortals call');
    if (!_.isFunction(callback)) callback = function (){};

    var reqUrl = url.resolve('http://localhost:7777', contextName + '/portals.xml');
    ...
    request({
        ...
        parseString(body, {explicitArray: false}, function (err, xml) {
            if (err || !xml.portals) return util.err('Error requesting url ' + reqUrl + '. Server have not provided portals list.');
...
exports.getPortalModel = function (portalName, contextName, callback) {
    request({
        auth: {
            user: 'admin',
            pass: 'admin'
        ...
        method: 'GET',
        url: url.resolve('http://localhost:7777', contextName + '/export/portal?portalName=' + portalName)
    }, function (err, httpResponse, body) {
