...
bbscaff.prototype.request = function(config, request_callback){
	request(_.extend({
		...
		method: 'POST',
		url: url.resolve('http://localhost:7777', '/portalserver/catalog'),
		headers: {
...
bbscaff.prototype.generate = function(answers, destination_path, callback, filter, srcConfig){
        filter = ['.DS_Store'].concat(filter || []);
        for (var i = 0; i < filter.length; i++) {
            filter[i] = "!" + this.template_dir + "/" + filter[i];
        }
        var src = [this.template_dir + '/**/*'].concat(filter);
	var excludedImages = gulpFilter(['**/*', '!**/*.jpg', '!**/*.png', '!**/*.gif'], srcConfig);

	gulp.src(src, srcConfig || {})
		.pipe(excludedImages)
