var path = require('path');
var inquirer = require('inquirer');
var _ = require('lodash');
var util = require('../lib/util');
var spawn = require('cross-spawn').spawn;
var git = require('gift');
var Q = require('q');

//Gulp Modules
var gulp = require('gulp');
var template = require('gulp-template');
var rename = require('gulp-rename');
var conflict = require('gulp-conflict');
var gulpFilter = require('gulp-filter');

module.exports = {

    /**
     * Default template_dir is the templates root of the repo
     */
    template_dir: path.join(__dirname, '..', 'templates'),

    /**
     * Generate files based on input options and destination path
     * @param  {Object} answers Inquirer answers
     * @param  {String} options Destination path
     * @param  {Object} options Extended options
     */
    generate: function (answers, options, callback) {

        var opts = _.isString(options) ? {destination_path: options} : options;

        if (answers.widget && !answers.widget.transforms && answers.widget.name) {
            var lettersName = answers.widget.name.split('');
            lettersName[0] = lettersName[0].toUpperCase();
            answers.widget.transforms = {
                name: {
                    camelCase: answers.widget.name.toUpperCase(),
                    capitalizeFirstLetter: lettersName.join('')
                }
            };
        }
        options = _.extend({}, {
            template_dir: this.template_dir,
            destination_path: '',
            excludeFilter: ['.DS_Store'],
            srcOptions: {
                dot: true
            },
            interpolate: /<%=([\s\S]+?)%>/g
        }, opts);

        var src = [options.template_dir + '/**/*'].concat(options.excludeFilter.map(function (filter) {
            return '!' + options.template_dir + '/' + filter;
        }));

        var excludedImages = gulpFilter(['**/*', '!**/*.jpg', '!**/*.png', '!**/*.gif'], options.srcOptions);

        gulp.src(src, options.srcOptions)
            .pipe(excludedImages)
            .pipe(template(answers, {interpolate: options.interpolate}))
            .pipe(excludedImages.restore())
            .pipe(rename(function (path) {
                _.each(answers, function (answer, key) {
                    path.basename = path.basename.split('{' + key + '}').join(answer);
                });
            }))
            .pipe(conflict('./'))
            .pipe(gulp.dest(options.destination_path))
            .on('end', function () {
                if (_.isFunction(callback)) {
                    callback();
                }
            });
    },

    /**
     * Generate files based on an archetype
     */
    archetype: function (answers, options, callback) {
        var mvnArchetypeArgs = ['archetypeArtifactId', 'archetypeGroupId', 'archetypeVersion'].map(function (arg) {
            return '-D' + arg + '=' + options[arg];
        });

        var mvnProjectArgs = Object.keys(answers).map(function (arg) {
            return '-D' + arg + '=' + answers[arg];
        });

        var cmd = spawn('mvn', ['archetype:generate', '-DinteractiveMode=false'].concat(mvnArchetypeArgs).concat(mvnProjectArgs));
        cmd.stdout.pipe(process.stdout);

        cmd.on('exit', function () {
            if (typeof callback === 'function') {
                callback(answers);
            }
        });
    },

    /**
     * Fetches a template from git before
     */
    fetchTemplate: function (url, location, callback) {
        var deferred = Q.defer();
        callback = callback || function () {
            };
        var template_path = path.resolve(location, 'template');
        spawn('rm', ['-Rf', template_path])
            .on('exit', function () {
                git.clone(url, template_path, function (err) {
                    if (err) {
                        deferred.reject(err);
                        return callback(err);
                    }
                    spawn('rm', ['-Rf', path.resolve(template_path, '.git')])
                        .on('exit', function () {
                            deferred.resolve();
                        });
                });
            });
        return deferred.promise;
    },

    /**
     * Get current bundle name based on user current folder on terminal
     * @return {String} current bundle name
     */
    getCurrentBundle: function () {
        return process.cwd().split(path.sep).slice(-2)[0];
    },

    /**
     * Wraps and exposes Inquirer prompt method
     * @param  {Object}   questions
     * @param  {Function} callback
     */
    prompt: function (questions, callback) {
        var deferred = Q.defer();
        callback = callback || function () {
            };
        inquirer.prompt(questions, function (answers) {
            callback(answers);
            deferred.resolve(answers);
        });
        return deferred.promise;
    },

    /**
     * Util: Return a camel case version of a string
     * @param  {String} string E.g. my-string
     * @return {String}        E.g. MyString
     */
    toCamelCase: function (string) {
        return _.camelCase(string);
    },

    /**
     * Return prefix based on a string
     * @param  {String} string E.g. my-string
     * @return {String}        E.g. ms (first letter of each separation)
     */
    getPrefix: function (string) {
        return string.split(/[-_ \.]/).map(function (word) {
            return word.split('')[0];
        }).join('');
    },

    /**
     * Wraps chalk and fancy icons for error messages
     */
    logError: function () {
        return util.err.apply(this, arguments);
    },

    /**
     * Wraps chalk and fancy icons for success messages
     */
    logSuccess: function () {
        return util.ok.apply(this, arguments);
    }

};
