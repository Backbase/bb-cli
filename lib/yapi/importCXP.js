...
(function () {
    ...
    exports.startImport = importCXP.startImport = function (options) {
        Options = options || {};
        ...
        //TODO: This should be we search in CWD or with search string if present.
        //Get all import xml files and push into global aJXON array
            getFiles(Options.unattended || process.cwd() + '/test/import/5.5/**/*.xml').forEach(function (item) {
    ...
    var processItems = function () {
        ...
        function getDifferences(localItemPackage, remoteItemPackage) {
            ...
            if (!isEqual) {
                ...
                // interested in local new properties or field changes so we will compare  here
                if (difference.properties.property.length === 0 && difference.tags.tag.length === 0){
                    delete localItem.properties;
                    delete localItem.tags;
                    delete remoteItem.properties;
                    delete remoteItem.tags;

                    ignoreRemoteChangesOnly = _.isEqual(localItem, remoteItem);
                }
                ...
                difference = ignoreRemoteChangesOnly ? null : diffPackage;
            }
        ...
        function revealDifferences(localItem, remoteItem) {
            ...
            if (localItemChangesOnly.properties.property) {
                localItemChangesOnly.properties.property = [];
            }

            if (localItemChangesOnly.tags && localItemChangesOnly.tags.tag) {
            ...
            // if cleanup = ture - we don't want to delete these just update from local only (default)
            // if cleanup = false - we will skip removing remote properties
            if (!Options.cleanup || Options.cleanup === true) {
